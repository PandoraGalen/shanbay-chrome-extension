...
(function () {
    ...
    var body_H = $(window).height();
        ...
        content_H = $('.content__main').outerHeight();
    
    $('header').css('zIndex', 1);
    $('.content__main').css('zIndex', 100);
    
    var _h = body_H - 33;
    var n = Math.ceil( (header_H + content_H) / _h );
    
    createNav();
    function createNav() {
        var bottom_nav = $('<div id="bottom_nav"></div>');
        $('body').append( bottom_nav );
           
        var lists = '<ul>';
        for (var i = 0; i < n; ++i) {
            lists += "<li><span>" + (i + 1) + "</span></li>";
        }
        lists += "</li>";
        $('#bottom_nav').append( lists );
        
        $('#bottom_nav ul li span').on('click', function() {
            var n = $(this).html();
